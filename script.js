// --- 1. GLOBAL SETUP (Must happen first) ---
var game_mode = 'waiting_for_website'; 
var myCanvas;
var ctx;

// This function must be Global so the HTML button can see it
window.startGame = function() {
    ...
    if(landing) {
        ...
        if (flapSound.context && flapSound.context.state === 'suspended') {
            flapSound.context.resume();
        }
    }
}

// --- 2. GAME INITIALIZATION (Waits for page to load) ---
window.onload = function() {
    // FIX: Properly get the canvas element
    ...
    const SUPER_JUMP_AMOUNT = -3.8;       
    const SUPER_ACCELERATION = 0.18;      
    const SUPER_MAX_FALL_SPEED = +3.5;    
    ...
    // FIX: Safety check for localStorage (prevents crash in private tabs)
    try {
        if (localStorage.getItem('flappyStars')) {
            collectedStars = parseInt(localStorage.getItem('flappyStars'));
        }
    } catch(e) { console.log("Local Storage not available"); }

    let availableSkins = ['default'];
    try {
        availableSkins = JSON.parse(localStorage.getItem('flappySkins')) || ['default'];
    } catch(e) {}
    ...
    function spawnChocolate() {
        ...
        ch chocolates.push(choc);
    }

    // Fix typo in original code: 'chococolates' -> 'chocolates'
    function spawnChocolate() {
        let choc = new MySprite();
        choc.MyImg = CHOCO_IMG;
        choc.x = boss.x;
    ...
    function purchaseOrEquipSkin(skinKey) {
        ...
        if (availableSkins.includes(skinKey)) {
            if (!skin.current) {
                ...
                try { localStorage.setItem('currentSkin', skinKey); } catch(e){}
    ...
    function Got_Player_Input(MyEvent) {
        let clickX = 0;
        let clickY = 0;
        // Logic to determine if it's a touch or mouse event to get coordinates
        if (MyEvent.type === 'touchstart' || MyEvent.type === 'mousedown') {
            ...
            // If undefined (e.g. touch ended), return
            if (clientX === undefined) return;

            // Scale coordinates because the canvas might be resized by CSS
        ...
        case 'boss': 
            if (MyEvent.type === 'touchstart' || MyEvent.type === 'mousedown') {
                // MOBILE: Check if top-left was tapped for Shop (Only when score < 1)
                if (clickX < 100 && clickY < 100 && currentScore < 1) {
                    game_mode = 'shop';
                    return;
                }
                // MOBILE: Check if top-right was tapped for Ability (D)
                if (clickX > myCanvas.width - 100 && clickY < 100) {
                    useWallDestruction();
                    return;
                }
            ...
            break;
        case 'shop':
            // MOBILE: Exit Shop tap (bottom of screen)
            if ((MyEvent.type === 'touchstart' || MyEvent.type === 'mousedown') && clickY > myCanvas.height - 60) {
                    game_mode = 'running'; break; 
            }
            // MOBILE: Cycle through skins by tapping the middle
            if (MyEvent.type === 'touchstart' || MyEvent.type === 'mousedown') {
                const skinKeys = Object.keys(SKINS);
                lastSkinIndex = (lastSkinIndex + 1) % skinKeys.length;
                purchaseOrEquipSkin(skinKeys[lastSkinIndex]);
            }
            // DESKTOP: Keep original keyboard logic
    ...
    // Attach listeners to window or canvas
        ...
        for (var i = 0; i < pipes.length; i++) {
            ...
            for (let i = stars.length - 1; i >= 0; i--) {
                ...
                if (collected) {
                    ...
                    try { localStorage.setItem('flappyStars', collectedStars); } catch(e){}
                    stars.splice(i, 1);
                    playPowerUpSound();
                }
            }
    ...
    function display_score() {
        ...
        if (currentScore < 1) {
            ctx.font = '14px Arial'; ctx.fillStyle = 'cyan';
            ctx.fillText('Tap top-left for Shop', 10, 85);
        } else {
    ...
    function display_lives() {
        ...
        if ((currentSkinKey === 'yellow' || currentSkinKey === 'black') && wallDestroyCharges > 0) { 
            ctx.strokeStyle = 'black';
            ctx.lineWidth = 3;
            ctx.strokeText('Charges: ' + wallDestroyCharges, 10, y);
            ctx.fillStyle = 'orange';
            ctx.fillText('Charges: ' + wallDestroyCharges, 10, y);
        }
    }
    ...
    function display_shop() {
        ...
        for (let i = 0; i < skinKeys.length; i++) {
            ...
            if (availableSkins.includes(key)) {
                status = skin.current ? ` [EQUIPPED]` : ` [OWNED]`;
                ctx.fillStyle = skin.current ? 'green' : 'blue';
            } else {
                status = ` [COST: ${skin.cost}]`;
        ...
        ctx.fillText('Tap Screen to Cycle Skins', myCanvas.width / 2, myCanvas.height - 70);
        ctx.fillText('Tap BOTTOM to Exit Shop', myCanvas.width / 2, myCanvas.height - 30);
    }
    ...
    pipe_piece.onload = reset_game; 
    pipe_piece.src = 'wall.png';
